...
## 2.Goals
"Private colleges vs. public colleges", there is a face-off that's been ongoing for many years. The major difference between public and private colleges lies in how they are funded. Public colleges are funded mostly by state governments, whereas private colleges rely heavily on private contributions, donations and tuition. The goal of our analysis is to compare public institutions and private colleges in terms of their "performance".  A college degree or post-secondary certificate is more important than ever. However, many students are not confident enough to making decision on the type of school they should choose. Our analysis aims at helping students understand the differences between public institutions and private colleges in terms of school size, school performance, tuition cost, degrees awarded, students' background and geographic information. The analysis should give potential students a brief overview of these two types of schools. 
...
1. Import and prepare data set. 
This step imports data set from local directory into R and read the file as csv file. All the necessary packages are installed in the R for later analysis and data visualization. Since we are interested in 26 variables, we select 26 variables from original data set and create a new data set for further analysis. The data manipulation is not implemented in the original data set. 

2. Data preparation. 
In this step, we clean our data set and modify variables. Missing values for each variable is computed to ensure that variables chosen have less than 20% missing values; data type is transferred into the proper type for analyzing convenience; the target variable Type, Private or Public is code as 0 for private and 1 for public; some independent variables are renamed for interpretation purpose. This step cleans and prepares all the data for further analysis. Dplyr is widely used in this step. 

3. Data summary
In this step, basic statistical analysis is conducted. The mean, median, and the variance is calculated for each numeric variable. This can help with the general understanding of the data set. `dplyr` package is widely used in this step. 
...
5. Conclusion
This step summarizes the descriptive results for private colleges and public institutions. The descriptive analysis indicates the difference between private school and public school from perspectives such as students' background, degree awarded, tuition and etc. The result can help students understand private and public school better and help them make decision for their education goal. 
...
csbop.df %>%
  ...
  {.} -> csb.vars.df
```
The resulting data frame `csb.vars.df` contains only the selected variables. Next we examine the structure of this new data frame `csb.vars.df`.
...
We observe a negative correlation between the family income of students and the percentage of first generation students. This tell us that for the most popular Certificate degrees in the most popular states California, New York and Texas, first generation students have lower family income. 
...
**Conclusion**: We could notice that inside of the variable group CUML_DEBT_N, CDR3_DENOM and DEBT_N, any two variables have a  strong positive linear correlation.
