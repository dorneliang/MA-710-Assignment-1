...
## 2.Goals
"Private colleges vs. public colleges". There is a face-off that has been ongoing for many years. The major difference between public and private colleges lies in how they are funded. Public colleges are funded mostly by state governments, whereas private colleges rely heavily on private contributions, donations and tuition. The goal of our analysis is to compare public institutions and private colleges in terms of their "performance."  A college degree or post-secondary certificate is more important than ever. However, many students are not confident enough of making decision on the type of school they should choose. Our analysis aims to help students understand the differences between public institutions and private colleges in terms of school size, school performance, tuition cost, degrees awarded, students' background and geographic information. The analysis should give potential students a brief overview of these two types of schools. 
...
* Find out how much the tuition differs between the two type of colleges
* Use selected variables to explore whether public colleges and private ones are really different in terms of repayment, aid and earnings perspective
* Understand how the percentage of degrees awarded between public school and private school differ. The `gglot` and `dplyr` functions to calculate mean, median, and variance are conducted. This can help students understand how those degrees are awarded between public schools and private schools and give them basic insights on making decisions. 
...
Then we read the csv file `MERGED2014_15_PP.csv` into the data frame `csb.df` using the `read_csv` function of the `readr` package.

```{r,message=FALSE, warning=FALSE}
csb.df = read_csv(paste0('/Users/student/Desktop/MA710/Tuesday version/Assignment 1/', 'MERGED2014_15_PP.csv'))
...
Since we would like to apply this function to a range of variables, we use the `summarise_at` command. `Summarise_at` takes the set of columns to be summarized as the first parameter, which in our case would be all columns, and the function to be applied to them as the second parameter.
...
The variables `NTP4_PRIV` and `NTP4_PUB` describe the average net price of either Public or Private colleges and are complementing each other. For this reason, we will build a new variable `NPT4` which describes the average net price for both public and private colleges.
...
The output tells us that there are 2302 private institutions providing predominantly Certificate degrees, 532 private institutions providing predominantly Associate degrees, 1469 private institutions providing predominantly Bachelor degrees and 269 private institutions providing predominantly Graduate degrees.

Similarly the figures for public institutions are 602 providing predominantly Certificate degrees, 766 providing predominantly Associate degrees, 590 providing predominantly Bachelor degrees and 16 private institutions providing predominantly Graduate degrees. The largest share (2302 institutions) is that of private institutions providing predominantly Certificate degrees.
...
Finally we summarized the variables in the academics category. The variables chosen were the percent of degrees awarded in 
...
csb.vars.df %>% 
  ...
  summarise_if(is.numeric, funs(mean, median, var), na.rm=TRUE)
```
The results indicated that the averages of the percentage of awarded degrees in those six fields are less than 1%. Among those six awarded degrees, the mean of the PCIP09 is the largest, regardless of the control type. This result indicates that among those six variables, percentages of degrees awarded in Communication are awarded more than other degrees. The variance of those six variables are relatively small, which means that from 2014 to 2015 the percentage of those six awarded degrees are almost the same. It does not vary too much in different institutes. PCIP01, PCIP03, PCIP05, PCIP09 degrees are awarded more in public schools than private schools. 
...
Based on the graph, we can observe that the colleges are not equally distributed among regions. Some regions such as numbers 2 (Mid East), 3 (Great Lakes) and 5 (Southeast) have a considerably larger amount of colleges compared to 7 (Rocky Mountains) and 9 (Outlying areas). In addition, the ratio of the public vs private colleges changes between regions. Regions such as 9 (outlying areas) have relatively high private vs public ratios while other regions such as 5 (Southeast) have more balanced representation of both Public and Private colleges.

The variable `CCBASIC` describes the Carnegie classification of colleges. There are 33 different type of colleges ranging between Associate's colleges and Doctoral Universities. We will explore the distribution of colleges based on the CCBASIC values.
...
The result indicates that the majority percentage of Ethnic degree in private school is less than 0.1 percent while it is less than 0.05 in public school. There is a higher percentage of Ethnic degree awarded in public school because public school has more outlines.
...
The result indicates that the majority of percentage of communication/journal degree awarded is less than 0.025 in public school. Private school has a higher percentage communication degress awarded and has more outliers.
...
#### Correlation Matrix
The correlation matrix is done among all the significant independent numeric variables. The purpose is to test whether there is high correlations among those siginificant variables. The code prepares all the numeric independent variables using as.numeric function. The correlation matrix function in `dplyr` is applied to get the correlation among independent variables. 
```{r,message=FALSE, warning=FALSE}
csb.ren.df %>%
  select(Type,Female,Ind_Per,First_Gen,HIGHDEG,NUMBRANCH,CCBASIC,CDR3_DENOM,CDR3,PCTFLOAN, DEBT_N,PCIP03,PCIP10) %>%
  {.} -> csb.vars1.df
 ...
 csb.vars1.df%>%
  mutate(Type=as.numeric(Type),
         ...
         HIGHDEG=as.numeric(HIGHDEG),
         NUMBRANCH=as.numeric(NUMBRANCH),
         CCBASIC=as.numeric(CCBASIC),
         CDR3_DENOM=as.numeric(CDR3_DENOM),
         CDR3=as.numeric(CDR3),
         PCTFLOAN=as.numeric(PCTFLOAN),
         DEBT_N=as.numeric(DEBT_N), 
...
cor_matrix1
```
For private school, "Ind_Per"" has 0.6 correlation with "First_Gen" and "HIGHDEG". For both private and public school, HIGHDEG has high correlation with CCBASIC as 0.89. For both private and public school, "CDR3_DENOM" has high correlation with "DEBT_N" as 0.97. All the other significant variables don't have high correlations with other significant variables. Understanding the correlation among those significant independent variables can help with selecting proper models in later steps. 

##Conclusion

Based on the exploratory analysis of the selected variables, we can say that there are some clear distinctions between public and private colleges. Public and private colleges tends to offer different types of degrees among each other based on the Carnegie Classification. Additionally, the degree concentration varies between the two types of colleges with higher number of Architectural and Communication technologies degrees offered at private colleges compared to public colleges and at the same time higher number of Ethic and Agricultural degrees offered at public Colleges. The tuition is another major difference between public and private, with higher tuition at Private colleges where we can also observe an extreme outliers indicating very expensive private institutions. There is no clear correlation between the tuition of the college and the income level of the attending students with some very low income students attending private schools and high income students preferring to enroll at public colleges. This could be explained by the higher level of federal and Pell grants which are given to students attending Private schools. Even though the private schools are more expensive compared to the Public colleges, students from lower-income households receive a variety of grants which allow them to afford the expensive institutions. The income level does affect the degree that the students are enrolled in. Our analysis indicates that students with the highest income strongly prefer Bachelor degrees while students with lower income are usually enrolled at Associate degrees or Certificate programs which could be explained by the significantly higher costs for Bachelor degrees compared to the rest. Private and Public colleges are also not spread out evenly in different regions of the country. That may be explained by the differences in the income levels and differences in the type of education needed in different parts of the country.

Overall, we can conclude that Private and Public colleges complement each other. They serve different needs of the population in terms of type of degrees and concentrations offered as well as more affordability which comes at lower tuition for the Public colleges and more grants available for Private colleges.
